...
impl From<mongodb::bson::de::Error> for MongoAggregateError {
...
mod tests {
    ...
    #[test]
    fn test_maps_bson_deserialization_error() {
        let bson_error =
            mongodb::bson::from_bson::<u32>(mongodb::bson::bson!({ "foo": "bar" })).unwrap_err();
        let mongo_error: MongoAggregateError = bson_error.into();
        assert!(matches!(
            mongo_error,
            MongoAggregateError::DeserializationError(_)
        ));
    }

    #[test]
    fn test_maps_bson_value_access_error() {
        let doc = mongodb::bson::doc! { "foo": "bar" };
        let access_error = doc.get_i32("foo").unwrap_err();
        let mongo_error: MongoAggregateError = access_error.into();
        assert!(matches!(
            mongo_error,
            MongoAggregateError::DeserializationError(_)
        ));
